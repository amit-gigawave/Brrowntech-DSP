...
export default function Home() {
  return (
    <div className="relative min-h-screen bg-[radial-gradient(circle_at_top_left,rgba(255,204,0,0.22),transparent_32%),radial-gradient(circle_at_top_right,rgba(255,153,0,0.16),transparent_28%),linear-gradient(180deg,#0a0a00_0%,#050500_45%,#000000_100%)]">
      <div className="pointer-events-none flex absolute inset-0 grid-bg opacity-40" />
      <div className="pointer-events-none absolute -left-16 top-12 h-72 w-72 rounded-full bg-yellow-500/15 blur-3xl" />
      <div className="pointer-events-none absolute right-0 top-0 h-80 w-80 rounded-full bg-yellow-500/12 blur-3xl" />
      <div className="pointer-events-none absolute bottom-0 left-1/4 h-72 w-72 rounded-full bg-orange-400/10 blur-3xl" />

      <header className="sticky top-0 z-50 border-b border-white/5 bg-[#0a0a00]/80 px-4 py-3 backdrop-blur-3xl md:px-8 md:py-5">
        <nav className="mx-auto flex max-w-7xl items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="relative flex h-12 w-12 items-center justify-center rounded-2xl bg-yellow-500/20 text-yellow-400 shadow-[0_0_30px_rgba(250,204,21,0.15)] ring-1 ring-yellow-400/20">
              <div className="absolute inset-0 animate-pulse rounded-2xl bg-yellow-400/5 blur-lg" />
              <Waves className="relative h-6 w-6" />
            </div>
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.3em] text-yellow-400/70 md:text-xs">Brrowntech</p>
          ...
          <div className="hidden md:flex items-center gap-8 mr-8">
            {["Overview", "Interface", "Specs", "Support"].map((item) => (
              <a key={item} href="#" className="text-xs font-bold uppercase tracking-widest text-white/40 transition-colors hover:text-yellow-400">{item}</a>
            ))}
          </div>
      ...
      <main className="relative z-10 mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 pb-safe pt-6 md:gap-10 md:px-8 md:pt-14">
        <section className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="flex flex-col justify-center rounded-[2.5rem] border border-white/10 bg-white/4 p-6 shadow-[0_32px_100px_-20px_rgba(0,0,0,0.5)] md:p-12">
            <div className="inline-flex items-center gap-2 rounded-full border border-yellow-400/20 bg-yellow-500/10 px-4 py-1.5 text-[10px] font-black uppercase tracking-[0.25em] text-yellow-200">
            ...
            <h1 className="mt-6 text-4xl font-black leading-[0.9] tracking-tight text-white sm:text-5xl md:mt-8 md:text-7xl lg:text-8xl">
              Precision audio
              <span className="block mt-2 bg-gradient-to-r from-yellow-400 via-yellow-500 to-orange-400 bg-clip-text text-transparent italic">
                reimagined
              </span>
            </h1>
          ...
          <div className="order-first lg:order-last flex flex-col items-center justify-center rounded-[2.5rem] border border-yellow-500/20 bg-[linear-gradient(225deg,rgba(255,204,0,0.08),rgba(0,0,0,0.1))] p-6 shadow-[0_32px_100px_-20px_rgba(0,0,0,0.6)] md:p-10">
            <div className="relative mb-8 flex h-24 w-24 items-center justify-center md:h-32 md:w-32">
              <div className="absolute inset-0 animate-slow-spin rounded-full border-2 border-dashed border-yellow-500/20" />
              <div className="absolute inset-4 animate-reverse-spin rounded-full border border-white/5" />
              <div className="relative flex h-16 w-16 items-center justify-center rounded-3xl bg-yellow-500/20 text-yellow-400 shadow-[0_0_40px_rgba(250,204,21,0.2)] md:h-20 md:w-20">
                <Bluetooth className="h-8 w-8 md:h-10 md:w-10" />
              </div>
            </div>
            
            <div className="text-center">
              <p className="text-[11px] font-black uppercase tracking-[0.3em] text-yellow-400">Step 1: Link</p>
